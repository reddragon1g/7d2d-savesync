...
public class XUiC_ContinueGame : XUiC_NewContinueBase
{
	...
	public bool SavePlayable
	{
		...
		get
		{
			return (savesList?.SelectedEntryData?.Playable).GetValueOrDefault();
		}
	}
	...
	[PublicizedFrom(EAccessModifier.Private)]
	public void SelectSaveEntry(string _gameWorld, string _gameName, UserDataStorageType _storageType)
	{
		if (!savesList.TrySelectEntry(_gameWorld, _gameName, _storageType))
		{
	...
	public void deleteSaveWithConfirmation()
	{
		...
		XUiC_MessageBoxWindowGroup.ShowConfirmCancel(xui, Localization.Get("xuiDeleteSaveGame"), string.Format(Localization.Get("xuiSavegameDeleteConfirmation"), saveEntry.SaveName), "", Confirm, Cancel, 1.5f, _openMainMenuOnClose: false, _modal: false);
	...
	public override void OnDataManagementWindowClosed()
	{
		...
		{
			gameWorld = selectedEntryData.WorldName;
			gameName = selectedEntryData.SaveName;
			storageType = selectedEntryData.StorageType;
		}
		savesList.RebuildList();
		SelectSaveEntry(gameWorld, gameName, storageType);
		RefreshBindings();
	}
	...
	{
		EPlatformLoadSaveGameState state = PlatformApplicationManager.LoadSaveGame.GetState();
		switch (state)
		{
		case EPlatformLoadSaveGameState.ContinueGameSelect:
		{
			string gameWorld = PlatformApplicationManager.LoadSaveGame.GameWorld;
			string gameName = PlatformApplicationManager.LoadSaveGame.GameName;
			UserDataStorageType saveStorageType = PlatformApplicationManager.LoadSaveGame.SaveStorageType;
			XUiC_SavegamesList.ListEntry selectedEntryData = savesList.SelectedEntryData;
			if (selectedEntryData == null || !selectedEntryData.WorldName.EqualsCaseInsensitive(gameWorld) || !selectedEntryData.SaveName.EqualsCaseInsensitive(gameName))
			{
				if (!savesList.TrySelectEntry(gameWorld, gameName, saveStorageType))
				{
					PlatformApplicationManager.LoadSaveGame.SetFailed();
					break;
				}
			...
			{
				PlatformApplicationManager.LoadSaveGame.AdvanceStateFrom(state);
			}
			break;
		}
