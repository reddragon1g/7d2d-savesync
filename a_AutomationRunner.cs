...
public class AutomationRunner
{
	...
	public IEnumerator ExecutePerfSession(AutomationScript script, int startIndex, int endIndex)
	{
		...
		Log.Out($"[AutomationRunner] 컴 PerfSession: {runs} run(s) x {num} capture(s)/run 컴");
		for (int run = 0; run < runs; run++)
		{
			...
			for (int i = startIndex + 1; i < endIndex; i++)
			{
				...
				yield return ExecuteStep(script.steps[i], run, sessionDir);
			}
		}
		Log.Out("[AutomationRunner] 컴 PerfSession complete 컴");
	}
	...
	public IEnumerator ExecuteStep(AutomationStep step, int runIndex, string sessionDir)
	{
		...
		case AutomationStep.StepType.PreparePlayer:
		{
			...
			player3.IsGodMode.Value = true;
			player3.IsNoCollisionMode.Value = true;
			player3.IsFlyMode.Value = true;
			player3.IsSpectator = true;
			player3.Buffs.AddBuff("god");
		...
		{
			EntityPlayerLocal player6 = GetPlayer();
			if (player6 == null)
			{
				Log.Error("[AutomationRunner] WaitForChunksLoaded: no player.");
				break;
			}
			ChunkManager.ChunkObserver chunkObserver = player6.ChunkObserver;
		...
		case AutomationStep.StepType.MoveLine:
		{
			...
			if (step.hasStart)
			{
				player5.SetPosition(step.positionB);
			}
			bool done = false;
			player5.EnableAutoMove(_enable: true).StartLine(step.duration, 0, step.position, [PublicizedFrom(EAccessModifier.Internal)] () =>
			{
				done = true;
			});
			while (!done)
		...
		case AutomationStep.StepType.Orbit:
		{
			...
			bool done2 = false;
			player4.EnableAutoMove(_enable: true).StartOrbit(step.duration, 0, step.position, step.lookForward, step.isFlipped, [PublicizedFrom(EAccessModifier.Internal)] () =>
			{
				done2 = true;
			});
			while (!done2)
